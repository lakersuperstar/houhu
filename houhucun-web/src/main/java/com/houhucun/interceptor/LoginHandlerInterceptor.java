...
public class LoginHandlerInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object arg2) throws Exception {
		// 1、请求到登录页面 放行
		if (request.getServletPath().endsWith(loginUrl)) { return true; }
		// 2、TODO 比如退出、首页等页面无需登录，即此处要放行 允许游客的请求
		if (request.getServletPath() == null || request.getServletPath().equals("/") || request.getServletPath().endsWith(loginController)) { return true; }
		// 3、如果用户已经登录 放行
		...
		if (cks != null && cks.length > 0) {
			...
			if (StringUtils.isNotBlank(user) && StringUtils.isNotBlank(login)) {
				...
				if (StringUtils.isNotBlank(loginUser) && user.equals(loginUser.substring(0, loginUser.length() - 1))) {
